...
RemoteParticipant::RemoteParticipant(RemoteBroker *p_remoteBroker, GuidPrefix_t guid_prefix)
{
	b_end = false;
	this->p_remoteBroker = p_remoteBroker;
	...
	do {
		...
		case PID_METATRAFFIC_MULTICAST_LOCATOR:
			...
			if (is_pdp) {
				...
				isIPv6 = changeMulticast((Locator_t*)&p_data[i_used + sizeof(Parameter)], p_remoteBroker->bind_port);
			}
			break;
		case PID_VENDORID:
...
void RemoteParticipant::sendPDP(uint8_t *p_org_data, int i_org_size)
{
	...
	p_remoteBroker->send(p_senddata);
...
void RemoteParticipant::getData()
{
	...
	if (data->i_size > 1480)
	{
		printf("Maybe can't send data to remote Broker.\n");
	}
	...
	p_remoteBroker->sendToRemoteBroker(p_senddata);
