...
#define MAX_SAMPLES_WRITER 1
#define MAX_SAMPLES_READER 10
...
void on_data_available(DataReader* p_datareader)
{
	...
	if (foo_seq.i_seq)
	{
		while (foo_seq.i_seq)
		{
			...
			trace_msg(OBJECT(current_object(0)), TRACE_TRACE3, "### %s\t color[%d] : %s, x : %d, y : %d, shapesize : %d", __FUNCTION__, foo_seq.i_seq, shape_data.color.value, shape_data.x, shape_data.y, shape_data.shapesize);
			printf("### %s\t color[%d] : %s, x : %d, y : %d, shapesize : %d\r\n", __FUNCTION__, foo_seq.i_seq, shape_data.color.value, shape_data.x, shape_data.y, shape_data.shapesize);
	...
	{
		trace_msg(OBJECT(current_object(0)), TRACE_TRACE3, "### %s\t ********** on_data_available -> no data.", __FUNCTION__);
		printf("### %s\t ********** on_data_available -> no data.\r\n", __FUNCTION__);
	}

	if (sample_info_seq.i_seq)
	{
		FREE(sample_info_seq.pp_sample_infos);
	}
}
...
void write_thread(void* p_dummy)
{
	...
	datawriter_qos.resource_limits.max_samples = MAX_SAMPLES_WRITER;
//	datawriter_qos.resource_limits.max_instances = 2;
//	datawriter_qos.resource_limits.max_samples_per_instance = 5;

	//DataWriter 생성
	p_datawriter = (FooDataWriter *)p_publisher->create_datawriter(p_publisher, p_topic, &datawriter_qos, NULL, ALL_ENABLE_STATUS);
	trace_msg(OBJECT(current_object(0)), TRACE_TRACE3, "History.kind: [%d][%d]\r\n", p_datawriter->datawriter_qos.history.kind, ((rtps_writer_t*)p_datawriter->p_related_rtps_writer)->p_writer_cache->history_kind);
	...
	//msleep(3000000);//이게 없으면 WH1:RH1 통신이 안된다.
...
void read_thread(void* p_dummy)
{
	...
	datareader_qos.resource_limits.max_samples = MAX_SAMPLES_READER;
//	datareader_qos.resource_limits.max_instances = 2;
...
void main()
{
	...
	//////////////////////////////////////////////////////////////////////////
	// 내 로컬 IP 주소가 192.168.72.129 이면, reader_thread를 실행하고,
