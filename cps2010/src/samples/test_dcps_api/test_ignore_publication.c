//
//	이파일은 ignore_publication() API를 테스트하기 위한 파일이며
//  get_matched_publications(), get_matched_publication_data()를 함께 검중한다.
//  DataReader의 on_data_available Listener를 통해 sample을 수신하다 BULE 데이터가 오면 ignore
//	작성자 : jeman
//
//	이력
//	2014-04-10 작성
...
void on_data_available(DataReader* p_datareader)
{
	...
	DomainParticipant* p_domain_participant=p_datareader->p_subscriber->p_domain_participant;
	...
	////////////////////////ignore_publication TEST//////////////////////////////////
	if (strcmp(shape_data.color.value,"BLUE") == 0)
	{		
		p_foo_datareader->get_matched_publications((DataReader*)p_foo_datareader, &InstanceHandleSeq);
		...
		if (InstanceHandleSeq.i_seq)
		{
			...
			if (handle > 0)
			{
				...
				ret = p_foo_datareader->get_matched_publication_data((DataReader*)p_foo_datareader, &publication_data, handle);

				if (ret == RETCODE_OK)
				{
					printf("Topic name of Publication is %s \n",publication_data.topic_name);
					printf("Type name of Publication is %s \n",publication_data.type_name);
				}

				FREE(publication_data.topic_name);
				FREE(publication_data.type_name);
				p_domain_participant->ignore_publication(p_domain_participant, handle);
				printf("Publications are ignored");
			}
		}
