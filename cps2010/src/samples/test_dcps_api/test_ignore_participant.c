//
//	이파일은 ignore_participant() API를 테스트하기 위한 파일이며
//  get_discovered_participant(), get_discovered_participant_data()를 함께 검중한다.
//	작성자 : jeman
//
//	이력
//	2014-04-10 작성
...
void write_thread(void* p_dummy)
{
	...
	while (!b_is_quit && !is_quit())
	{
		...
		if(InstanceHandleSeq.i_seq)
		{
			...
			handle = *InstanceHandleSeq.pp_instancehandle[1];
			if(handle > 0)
			{
				...
				p_domainparticipant->ignore_participant(p_domainparticipant, handle);
				printf("Participants are ignored");
			}
		}
