...
/*
	...
	2014-03-04 : on_sample_rejected 함수 추가, 김경일
*/
...
/* 변수 2개 출력하는 함수                                               */
/************************************************************************/
bool compare_variables(const char* const p_variable_name, const int32_t local_variable, const int32_t global_variable)
{
	bool b_compare = (local_variable == global_variable);

	printf("### %s ", p_variable_name);
	printf("[%3d] ", local_variable);
	if (b_compare)
	{
		printf("== ");
	}
...
{
	SampleRejectedStatus* p_local_status;
	int32_t total_count = p_status->total_count;
	int32_t total_count_change = p_status->total_count_change;
	int32_t last_reason  = p_status->last_reason;
	int32_t last_instance_handle  = p_status->last_instance_handle;

	p_local_status = (SampleRejectedStatus*)calloc(1, sizeof(SampleRejectedStatus));

	p_datareader->get_sample_rejected_status(p_datareader, p_local_status);

	compare_variables("total_count", p_local_status->total_count, total_count);
	compare_variables("total_count_change", p_local_status->total_count_change, total_count_change);
	compare_variables("last_reason", p_local_status->last_reason, last_reason);
	compare_variables("last_instance_handle", p_local_status->last_instance_handle, last_instance_handle);
...
void create_domainparticipant(void)
{
	...
	//p_domainparticipant_listener->on_sample_rejected = on_sample_rejected;
...
void write_thread(void* p_dummy)
{
	...
	datawriter_qos.reliability.max_blocking_time.sec = 1;
...
void read_thread(void* p_dummy)
{
	...
	datareader_qos.reliability.max_blocking_time.sec = 1;
	datareader_qos.reliability.max_blocking_time.nanosec = 0;
	datareader_qos.history.kind = KEEP_ALL_HISTORY_QOS;
	datareader_qos.history.depth = 3;
	datareader_qos.resource_limits.max_samples = 3;
	datareader_qos.resource_limits.max_instances = 1;
