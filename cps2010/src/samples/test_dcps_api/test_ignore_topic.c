//
//	이파일은 ignore_topic() API를 테스트하기 위한 파일이며
//  get_discovered_topic(), get_discovered_topic_data()를 함께 검중한다.
//	작성자 : jeman
//
//	이력
//	2014-04-10 작성
	...
	while (!b_is_quit && !is_quit())
	{
		...
		if(InstanceHandleSeq.i_seq)
		{
			...
			if(handle > 0)
			{
				...
				p_domainparticipant->ignore_topic(p_domainparticipant, handle);
				printf("Topics are ignored");
			}
		}
