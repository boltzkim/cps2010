//
//	이파일은 timestamp관련 API를 테스트하기 위한 파일이며
//  register_instance_w_timestamp(), unregister_instance_w_timestamp(), write_w_timestamp(), dispose_w_timestamp() 를 함께 검중한다.
//  writer_thread에서 write 할 때 timestamp값을 현재 시간값으로 설정
//	작성자 : jeman
//
//	이력
//	2014-04-10 작성
...
void write_thread(void* p_dummy)
{
	...
	InstanceHandle_t instance_handle_timestamp;
	...
	while (!b_is_quit && !is_quit())
	{
		...
		instance_handle_timestamp = p_datawriter->register_instance_w_timestamp(p_datawriter, p_foo, p_current_time);
...
//		p_datawriter->unregister_instance_w_timestamp(p_datawriter, p_foo, instance_handle_timestamp, p_current_time);
		//p_domainparticipant->get_current_time(p_domainparticipant, &p_current_time);
		...
		//	p_datawriter->write_w_timestamp(p_datawriter, p_foo, 0, p_current_time);
		//}
	...
	p_domainparticipant->get_current_time(p_domainparticipant, &p_current_time);
	p_datawriter->unregister_instance_w_timestamp(p_datawriter, p_foo, instance_handle_timestamp, p_current_time);
	p_datawriter->dispose_w_timestamp(p_datawriter, p_foo, instance_handle_timestamp, p_current_time);
