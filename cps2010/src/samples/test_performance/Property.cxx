...
inline static const std::string & _getKey(ValueMapImpl * map, const std::string &key) {
    ...
    if (iter == map->end()) {
        char buf[128];
        SNPRINTF(buf, 128, "Key not found in ValueMap: %s", key.c_str());
...
// ---------------------------------------------------------------------------
void ValueMap::parse_as_duration(const std::string &valueIn, Duration_t *valueOut) {
    if (valueIn == "I") {
        if (valueOut) *valueOut = TIME_INFINITE;
        return;
    }

    char *ptr = NULL;
    unsigned long long usec = STRTOULL(valueIn.c_str(), &ptr, 10);
    if (*ptr) {
        std::ostringstream os;
    ...
    if (valueOut) {
        valueOut->sec = (signed int)(usec / 1000000ull);
        valueOut->nanosec = (unsigned int)((usec % 1000000ull) * 1000ull);
    }
}
...
void ValueMap::set_int(const std::string &key, int value) {
    ...
    SNPRINTF(buf, MAX_TEMP_PROPERTY_BUFFER, "%d", value);
...
void ValueMap::set_double(const std::string &key, double value) {
    if (_isReadOnly) throw read_only(key);
    char buf[MAX_TEMP_PROPERTY_BUFFER];
    SNPRINTF(buf, MAX_TEMP_PROPERTY_BUFFER, "%g", value);
    (*_theImpl)[key] = buf;
}

// ---------------------------------------------------------------------------
void ValueMap::set_duration(const std::string &key, const Duration_t &value) {
    if (_isReadOnly) throw read_only(key);
    char buf[MAX_TEMP_PROPERTY_BUFFER];
    if (value.sec== 0x7fffffff && value.nanosec == 0xffffffff) {
        // Represent infinite duration with "I" for now.
        SNPRINTF(buf, MAX_TEMP_PROPERTY_BUFFER, "I");
    } else {
        unsigned long long usec = value.sec * 1000000ull + value.nanosec / 1000ull;
        // Store as string in microsecond resolution.
        SNPRINTF(buf, MAX_TEMP_PROPERTY_BUFFER, "%llu", usec);
    }
    (*_theImpl)[key] = buf;
}
...
// ---------------------------------------------------------------------------
void ValueMap::get_duration(const std::string &key, Duration_t &durationOut) const {
