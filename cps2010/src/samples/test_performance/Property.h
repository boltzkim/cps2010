...
public:
    ...
    /*e @brief Converts a string into a DDS_Duration_t
      ...
      @throws invalid_type if the string cannot be converted to the given type
     */
    static void parse_as_duration(const std::string &valueIn, Duration_t *valueOut = NULL);
...
public:
    ...
    /*e @brief sets a DDS_Duration_t value for the key 
      ...
      @throws read_only - if map is in read-only state */
    void set_duration(const std::string &key, const Duration_t &value);
    ...
    void get_duration(const std::string &key, Duration_t &durationOut) const;
