...
void on_data_available(DataReader* p_datareader)
{
	...
	p_foo_datareader->read(p_datareader, &foo_seq, &sample_info_seq, 1, ANY_SAMPLE_STATE, ANY_VIEW_STATE, ANY_INSTANCE_STATE);
...
void write_thread(void* p_dummy)
{
	...
	p_publisher->delete_datawriter(p_publisher, p_datawriter);
}
