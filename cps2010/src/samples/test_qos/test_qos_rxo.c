...
/*
	QoS RxO 동작 시험 코드
	DataWriter 와 DataReader 의 QoS 값이 서로 호환되지 않을 경우 Connection 이 맺어지지 않는 것을 확인한다. 

	2010-07-22 : 시작
	...
	2013-11-22 : DataReader Listener 테스트 코드 삽입 및 정리, 김경일
	2013-12-02 : QoS RxO 동작 시험 코드 삽입, 김경일
*/
...
void on_offered_incompatible_qos(in_dds DataWriter* p_writer,in_dds OfferedIncompatibleQosStatus* p_status)
{
	printf("\r\n########## %s:%d, event triggered ...[%d]\r\n\r\n", __FUNCTION__, __LINE__, p_status->last_policy_id);
}


void on_requested_incompatible_qos(in_dds DataReader* p_reader, in_dds RequestedIncompatibleQosStatus* p_status)
{
	printf("\r\n########## %s:%d, event triggered ...[%d]\r\n\r\n", __FUNCTION__, __LINE__, p_status->last_policy_id);
}
...
//	datawriter_qos.latency_budget.duration.sec = 10;
...
#endif
//	datareader_qos.durability.kind = PERSISTENT_DURABILITY_QOS;
//	datareader_qos.deadline.period.sec = 1;
//	datareader_qos.latency_budget.duration.sec = 1;
