...
//		return;
	...
	p_foo_datareader->read(p_foo_datareader, &foo_seq, &sample_info_seq, 5, NOT_READ_SAMPLE_STATE, ANY_VIEW_STATE, ANY_INSTANCE_STATE);
...
void write_thread(void* p_dummy)
{
	...
	//Presentation QoS 설정(Writer side, 설정값에 따라 Test case가 바뀜)
	publisher_qos.presentation.access_scope = TOPIC_PRESENTATION_QOS;
	publisher_qos.presentation.coherent_access = true;
	publisher_qos.presentation.ordered_access = false;
...
//*
	...
	p_publisher->begin_coherent_changes(p_publisher);
	for(i=0;i<10;i++)
	{
		point(&data, i, 0); //point 입력 함수
		msleep(sleep_time);
		p_datawriter->write(p_datawriter, p_foo, 0);
	}
	msleep(sleep_time);
	p_publisher->end_coherent_changes(p_publisher);
...
void read_thread(void* p_dummy)
{
	...
	//Presentation QoS 설정(Reader side, 설정값에 따라 Test case가 바뀜)
	subscriber_qos.presentation.access_scope = TOPIC_PRESENTATION_QOS;
	subscriber_qos.presentation.coherent_access = true;
	subscriber_qos.presentation.ordered_access = false;
