...
/*
	Deadline QoS 동작 시험 코드.
	Deadline QoS 값이 서로 다를 경우 Connection 이 안맺어지는 것을 확인한다.

	2010-07-22 : 시작
	...
	2013-11-22 : DataReader Listener 테스트 코드 삽입 및 정리, 김경일
	2013-12-05 : Deadline QoS 동작 시험 코드 삽입, 김경일
*/
...
void write_thread(void* p_dummy)
{
	...
	while (!b_is_quit && !is_quit())
	{
		...
		if (j == 3)
		{
			printf("#################### %s:%d\tset_qos() calling \r\n", __FUNCTION__, __LINE__);
			datawriter_qos.deadline.period.sec = 2;
...
void read_thread(void* p_dummy)
{
	...
	datareader_qos.deadline.period.sec = 5;
