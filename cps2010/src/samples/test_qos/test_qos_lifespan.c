...
/*
	...
	2013-12-09 : Lifespan QoS 동작 시험 코드 삽입, 김경일
*/
...
void write_thread(void* p_dummy)
{
	...
	datawriter_qos.lifespan.duration.sec = 5;
	datawriter_qos.lifespan.duration.nanosec = 0;

	//DataWriter 생성
	p_datawriter = (FooDataWriter *)p_publisher->create_datawriter(p_publisher, p_topic, &datawriter_qos, NULL, ALL_ENABLE_STATUS);
//	((DataWriter*)p_datawriter)->get_qos((DataWriter*)p_datawriter, &datawriter_qos);
...
void read_thread(void* p_dummy)
{
	...
	((DataReader*)p_datareader)->get_qos((DataReader*)p_datareader, &datareader_qos);
}
