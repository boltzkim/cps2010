...
/*
	Partition QoS 동작 시험 코드.
	Partition QoS 값이 서로 다를 경우 Connection 이 안맺어지는 것을 확인한다.

	2010-07-22 : 시작
	...
	2013-11-22 : DataReader Listener 테스트 코드 삽입 및 정리, 김경일
	2013-12-05 : Partition QoS 동작 시험 코드 삽입, 김경일
*/
...
void write_thread(void* p_dummy)
{
	...
	INSERT_ELEM(publisher_qos.partition.name.pp_string, publisher_qos.partition.name.i_string, publisher_qos.partition.name.i_string, strdup("1234"));
	...
	while (!b_is_quit && !is_quit())
	{
		...
		if (j == 3)
		{
			printf("### %s:%d\tpublisher->set_qos() calling \r\n", __FUNCTION__, __LINE__);
			for (i = 0; i < publisher_qos.partition.name.i_string; i++)
				REMOVE_ELEM(publisher_qos.partition.name.pp_string, publisher_qos.partition.name.i_string, i);
			INSERT_ELEM(publisher_qos.partition.name.pp_string, publisher_qos.partition.name.i_string, publisher_qos.partition.name.i_string, strdup("1234"));
			p_publisher->set_qos(p_publisher,  &publisher_qos);
		}
	}
}
...
void read_thread(void* p_dummy)
{
	...
	INSERT_ELEM(subscriber_qos.partition.name.pp_string, subscriber_qos.partition.name.i_string, subscriber_qos.partition.name.i_string, strdup("123"));
