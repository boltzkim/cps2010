...
RemoteRouter::RemoteRouter(GuidPrefix_t _prefix, struct sockaddr_in addr)
{
	...
	printPrefix(_prefix);
	printf("\nNew RemoteRouter(Mapped) : %s:%d\n", mapped_ip.c_str(), mapped_port);
	isMapped = true;
	alive();
}

RemoteRouter::RemoteRouter(struct sockaddr_in addr)
{
	...
	printf("New RemoteRouter(Mapped) : %s:%d\n", mapped_ip.c_str(), mapped_port);
	isMapped = true;
	alive();

}

RemoteRouter::RemoteRouter(std::string ip, int port)
{
...
void RemoteRouter::setLocalAddr(struct sockaddr_in addr)
{
...
RemoteRouter::~RemoteRouter()
{
	...
	printf("Delete GOOD RemoteRouter\n");
...
void RemoteRouter::Network(RemoteRouter *p_RemoteRouter)
{
	while (p_RemoteRouter->is_end == false)
	{
		p_RemoteRouter->getData();
	}
}

void RemoteRouter::Live(RemoteRouter *p_RemoteRouter)
{
	while (p_RemoteRouter->is_end == false)
	{
		Sleep(LIVESLEEPTIME);

		p_RemoteRouter->sendLive();
	}
}

void RemoteRouter::sendLive()
{
		...
		|| data->i_size == getNotifyMessageSize())
	{
		if (memcmp(data->p_data, "ALIVE:", 6) == 0)
		{
			...
			}else if(memcmp(&remote_prefix, &this->remote_prefix, sizeof(GuidPrefix_t)) != 0)
			{
				...
				getRouter()->getRemoteRouterManager()->sendReNACK(client, remote_prefix);
			}
			else {
				sendACK();
			}
		...
		if (memcmp(data->p_data, "TOPAR:", 6) == 0)
		{
			...
			if (memcmp(&this->remote_addr, &client, sizeof(client)) == 0)
			{
				isfromremoteRouter = true;
				acked_time = currenTime();
			}

			if (isfromremoteRouter == false)
			{
...
void RemoteRouter::changeAddr(GuidPrefix_t prefix, struct sockaddr_in addr)
{
...
void RemoteRouter::parse(data_t	*data, bool isfromremoteRouter)
{
	Header		a_header;
	int32_t		ret;
	int32_t		i_pos = 0;
	...
	if (isfromremoteRouter)
	{
		...
		if (isPDP)
		{
			...
			for (std::list<InParticipant*>::iterator li = getRouter()->inParticipants.begin(); li != getRouter()->inParticipants.end(); li++)
			{
				...
				//{
				//	/*getRouter()->inParticipants.remove(_InParticipant);
				//	delete _InParticipant;

				//	if (getRouter()->inParticipants.size() == 0) break;
				//	li = getRouter()->inParticipants.begin();*/
	...
	if (isfromremoteRouter == false)
	{
		...
		data_t *p_senddata = data_new(data->i_size);
		memcpy(p_senddata->p_data, p_data, data->i_size);
		sendToRemoteRouter(p_senddata);
	}

}


RemoteParticipant *RemoteRouter::getRemoteParticipant(GuidPrefix_t a_guid_prefix)
{
...
void RemoteRouter::send(data_t *p_data)
{
...
void RemoteRouter::sendToRemoteRouter(data_t *p_data)
{
	//리모트 라우터로 보내기...
...
void RemoteRouter::sendDataToInParticipant(GuidPrefix_t in_prefix, uint8_t *p_org_data, int i_org_size)
{
