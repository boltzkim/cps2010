...
{
public:
	RemoteRouter(GuidPrefix_t prefix, struct sockaddr_in addr);
	RemoteRouter(struct sockaddr_in addr);
	RemoteRouter(std::string ip, int port);
	virtual ~RemoteRouter();


	void getData();
	void send(data_t *p_data);
	void sendToRemoteRouter(data_t *p_data);
	...
	static void Network(RemoteRouter *p_RemoteRouter);
	static void Live(RemoteRouter *p_RemoteRouter);
...
private:
	...
	void parse(data_t	*data, bool isfromremoteRouter);
